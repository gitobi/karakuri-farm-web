...
class DevicesWateringStats extends React.Component {
  constructor(props) {
    super(props);
  ...
  load(id = this.props.item.id, unit = this.state.selectedUnit) {
    if (id) {
      ...
      this.props.actions.loadDevicesWateringStats(id, unit)
      .then((result) => {
        // 最新統計日を選択状態にする
...
function mapStateToProps(state) {
  return  {
    statsMap: state.devicesWatering.get('statsMap').toJS(),
